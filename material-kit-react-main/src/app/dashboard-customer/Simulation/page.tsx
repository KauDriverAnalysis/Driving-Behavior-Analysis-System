    ...
    <Box sx={{ pb: 5 }}>
      ...
      {!simulationData ? (
        ...
        <Grid container spacing={3}>
          ...
          {/* My Accident Analysis Details */}
          <Grid item xs={12}>
            <Card sx={{ mb: 3, border: simulationData.summary.accident_detected ? '2px solid #f44336' : '2px solid #4caf50' }}>
              <CardContent>
                <Typography variant="h6" fontWeight="600" sx={{ mb: 2 }}>
                  My Trip Analysis Details
                </Typography>
                <Grid container spacing={3} alignItems="center">
                  <Grid item xs={12} sm={6} md={2.4}>
                    <Box sx={{ textAlign: 'center' }}>
                      <Typography variant="body2" color="text.secondary">Incident Type</Typography>
                      <Typography variant="h6" fontWeight="bold">
                        {simulationData.summary.accident_type || (simulationData.summary.accident_detected ? 'Possible Impact' : 'Normal Trip')}
                      </Typography>
                    </Box>
                  </Grid>
                  <Grid item xs={12} sm={6} md={2.4}>
                    <Box sx={{ textAlign: 'center' }}>
                      ...
                      <Chip 
                        label={simulationData.summary.accident_severity || (simulationData.summary.accident_detected ? 'Medium' : 'Low')}
                        color={
                          (simulationData.summary.accident_severity || 'Low') === 'High' ? 'error' :
                          (simulationData.summary.accident_severity || 'Low') === 'Medium' ? 'warning' : 'success'
                        }
                        size="small"
                      />
                    </Box>
                  ...
                  <Grid item xs={12} sm={6} md={2.4}>
                    <Box sx={{ textAlign: 'center' }}>
                      <Typography variant="body2" color="text.secondary">Time Detected</Typography>
                      <Typography variant="body1">
                        {simulationData.summary.accident_timestamp || 'N/A'}
                      </Typography>
                    </Box>
                  </Grid>
                  <Grid item xs={12} sm={6} md={2.4}>
                    <Box sx={{ textAlign: 'center' }}>
                      <Typography variant="body2" color="text.secondary">My Top Speed</Typography>
                      <Typography variant="h6" color={simulationData.summary.maxSpeed > 100 ? 'error.main' : 'inherit'}>
                        {simulationData.summary.maxSpeed.toFixed(0)} km/h
                      </Typography>
                    </Box>
                  </Grid>
                  <Grid item xs={12} sm={6} md={2.4}>
                    <Box sx={{ textAlign: 'center' }}>
                      ...
                      <Typography variant="h6" fontWeight="bold" color="primary.main">
                        {simulationData.summary.accident_confidence || 85}%
                      </Typography>
                    </Box>
                  </Grid>
                </Grid>
                ...
                <Grid container spacing={3}>
                  <Grid item xs={12} sm={6} md={3}>
                    <Paper sx={{ p: 3, textAlign: 'center', bgcolor: 'grey.50' }}>
                      <Typography variant="h4" fontWeight="bold">
                        {simulationData.summary.totalRecords.toLocaleString()}
                      </Typography>
                      <Typography variant="body2" color="text.secondary">
                  ...
                  <Grid item xs={12} sm={6} md={3}>
                    <Paper sx={{ p: 3, textAlign: 'center', bgcolor: 'grey.50' }}>
                      <Typography variant="h4" fontWeight="bold">
                        {simulationData.summary.avgSpeed.toFixed(0)}
                      </Typography>
                      <Typography variant="body2" color="text.secondary">
                        My Average Speed (km/h)
                      </Typography>
                    </Paper>
                  </Grid>
                  
                  <Grid item xs={12} sm={6} md={3}>
                    <Paper sx={{ p: 3, textAlign: 'center', bgcolor: simulationData.summary.accident_detected ? 'error.50' : 'success.50' }}>
                      <Typography 
                        variant="h4" 
                        fontWeight="bold"
                        color={simulationData.summary.accident_detected ? 'error.main' : 'success.main'}
                      >
                        {simulationData.summary.accident_detected ? 'HIGH' : 'LOW'}
                      </Typography>
                      <Typography variant="body2" color="text.secondary">
                        My Risk Assessment
                      </Typography>
